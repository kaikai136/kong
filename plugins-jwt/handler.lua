...
local function retrieve_token(conf)
    ...
    if args[conf.uri_param_name] then
        return args[conf.uri_param_name]
    end

    local request_headers = kong.request.get_headers()
    local token_header = request_headers[conf.header_name]
...
local function do_authentication(conf)
    ...
    local ok_claims, errors = jwt:verify_registered_claims({ "exp" })
    ...
    if err then
        return kong.response.exit(500, { message = "An unexpected error occurred in Kong" })
    end
    ...
    -- check jti
    local ok, err = checkJti(conf, claims.jti)
    if not ok then
        return kong.response.exit(err.status, err.message)
    end
    -- add userId to header
    ok, err = checkUserIdAndAddHeader(conf, claims.uid)
    if not ok then
        return kong.response.exit(err.status, err.message)
    end
    -- add username to header
    ok, err = checkUserNameAndAddHeader(conf, claims.uname)
    if not ok then
        return kong.response.exit(err.status, err.message)
    end
    -- clear jwt
    if conf.clear_header_jwt then
        kong.service.request.clear_header(conf.header_name)
    end
    set_consumer(consumer, jwt_secret, token)
    return true
end
function checkUserIdAndAddHeader(conf, userId)
    if not userId then
        return false, { status = 401, message = "Invalid user_info in claims" }
    end

    if conf.add_header_uid then
        kong.service.request.add_header("x-uid", userId)
    end
    return true
end
function checkUserNameAndAddHeader(conf, username)
    if not username then
        return false, { status = 401, message = "Invalid user_info in claims" }
    end
    if conf.add_header_uname then
        kong.service.request.add_header("x-uname", username)
    end
    return true
end
-- TODO 增加redis校验，校验黑名单等操作
function checkJti(conf, jti)
    ...
    -- end
    -- -- get key
    -- local res, err = red:get(conf.blacklist_string_key_prefix .. jti)
    -- if err then
    --     kong.log.err("failed to get key", conf.blacklist_string_key_prefix .. jti)
    -- end
...
function JwtHandler:access(conf)
    ...
    if not conf.run_on_preflight_OPTIONS_request and kong.request.get_method() == "OPTIONS" then
        return
    end
